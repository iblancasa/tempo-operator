...
LD_FLAGS ?= "-X ${VERSION_PKG}.buildDate=${VERSION_DATE} -X ${VERSION_PKG}.version=${OPERATOR_VERSION} -X ${VERSION_PKG}.tempo=${TEMPO_VERSION}"
...
run: manifests generate fmt vet ## Run a controller from your host.
	...
	ENABLE_WEBHOOKS=false go run -ldflags ${LD_FLAGS} ./main.go start

.PHONY: docker-build
docker-build: ## Build docker image with the manager.
	docker buildx build --load --platform linux/${ARCH} -t ${IMG} --build-arg VERSION_PKG=${VERSION_PKG} --build-arg VERSION=${OPERATOR_VERSION} --build-arg VERSION_DATE=${VERSION_DATE} --build-arg TEMPO_VERSION=${TEMPO_VERSION} .
